...
def create_dataloaders(data_path: str, transform, batch_size: int):
    ...
    train_dataset = BraTSDataSet(cases=cases[:int(len(cases)*0.05)], transform=transform) #Contains 1000 cases at first, now 125
    #Creating a validation set
    valid_dataset = BraTSDataSet(cases=cases[int(len(cases)*0.1):int(len(cases)*0.11)], transform=transform) #Contains ~63
